...
import { supabaseClient, supabaseAdapter } from "../config/supabaseClient";
...
export class UserService {
  async getAllUsers(): Promise<User[]> {
    const { data, error } = await supabaseAdapter.from("users").select("*");
  ...
  async addUser(user: { name: string; email: string }): Promise<User> {
    const { data, error } = await supabaseAdapter.from("users").insert([user]);
