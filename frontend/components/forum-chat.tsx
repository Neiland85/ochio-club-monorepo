...
import type { ChatMessage, AnonymousUser, ForumChatProps, ChatStats } from "@/types/forum-chat"
...
export default function ForumChat({
  ...
  const [sampleUsers] = useState<AnonymousUser[]>(() => Array.from({ length: 8 }, () => generateAnonymousUser()))
  ...
  useEffect(() => {
    ...
    const initialMessages = Array.from({ length: 5 }, () => generateSampleMessage(sampleUsers))
    ...
    <Card className={`w-full max-w-4xl mx-auto ${className}`}>
      <CardHeader className="pb-3">
        ...
        {currentUser && (
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-muted-foreground">• Chat anónimo y seguro</span>
      ...
      <CardContent className="space-y-4">
              ...
              <div key={message.id} className="space-y-1">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className={`text-sm leading-relaxed ${message.isSystem ? "text-muted-foreground italic" : ""}`}>
                      {message.content}
                    </p>
                  </div>
                </div>
        ...
        <div className="text-xs text-muted-foreground text-center space-y-1">
          <p>💬 {stats.totalMessages.toLocaleString()} mensajes enviados • Presiona Enter para enviar</p>
          <p>🔒 Chat moderado automáticamente • Sin registro requerido • Respeta a otros usuarios</p>
        </div>
      </CardContent>
    </Card>
  )
