  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold mb-2">
            <span className={getSecurityScoreColor(metrics.securityScore)}>{metrics.securityScore}%</span>
          </div>
          <Progress value={metrics.securityScore} className="mb-2" />
          <Badge className={getSecurityScoreBadge(metrics.securityScore)}>
            {metrics.securityScore >= 90 ? "Excelente" : metrics.securityScore >= 70 ? "Bueno" : "Necesita Atención"}
          </Badge>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold text-red-600 mb-2">{metrics.blockedAttacks.toLocaleString()}</div>
          <p className="text-xs text-muted-foreground">
            De {metrics.totalRequests.toLocaleString()} solicitudes totales
          </p>
          <div className="mt-2">
            <Badge variant="destructive">
              {((metrics.blockedAttacks / metrics.totalRequests) * 100).toFixed(2)}% bloqueados
            </Badge>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold text-green-600 mb-2">{metrics.successfulLogins.toLocaleString()}</div>
          ...
          <div className="flex gap-2">
            ...
            <Badge variant="secondary">
              {((metrics.successfulLogins / (metrics.successfulLogins + metrics.failedLogins)) * 100).toFixed(1)}% éxito
            </Badge>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="text-2xl font-bold text-blue-600 mb-2">{metrics.csrfTokensGenerated.toLocaleString()}</div>
