...
import { RefreshCw, Search, CalendarIcon, Filter, AlertTriangle, Info, XCircle, Bug, Download } from "lucide-react"
import { format } from "date-fns"
import { es } from "date-fns/locale"
import type { SystemLogsProps, LogFilters, SystemLog } from "@/types/admin-settings"

export default function SystemLogsPanel({ logs, onRefresh, isLoading }: SystemLogsProps) {
  ...
  useEffect(() => {
    const filtered = logs.filter((log) => {
      ...
      const matchesDate = log.timestamp >= filters.dateRange.from && log.timestamp <= filters.dateRange.to
      const matchesSearch =
        filters.search === "" ||
  ...
  const getLevelBadge = (level: string) => {
    ...
    return <Badge variant={variants[level as keyof typeof variants] || "secondary"}>{level.toUpperCase()}</Badge>
  }
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Monitoreo y análisis de eventos del sistema en tiempo real</CardDescription>
          </div>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <Select
              value={filters.level}
              onValueChange={(value: any) => setFilters((prev) => ({ ...prev, level: value }))}
            >
              <SelectTrigger>
          ...
          {filteredLogs.map((log) => (
            <div key={log.id} className="border rounded-lg p-4 hover:bg-gray-50">
              <div className="flex items-start justify-between">
                ...
                <div className="text-xs text-muted-foreground">{format(log.timestamp, "dd/MM/yyyy HH:mm:ss")}</div>
              </div>
            </div>
          ))}
        </div>
