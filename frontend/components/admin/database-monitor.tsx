      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Estado y métricas de rendimiento de la base de datos</CardDescription>
          </div>
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-sm font-medium text-muted-foreground">Consultas/seg</p>
                  <p className="text-2xl font-bold">{metrics.performance.queriesPerSecond}</p>
                  <p className="text-xs text-muted-foreground mt-1">Promedio: {metrics.performance.avgQueryTime}ms</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{metrics.storage.usagePercentage}%</p>
                  <Progress value={metrics.storage.usagePercentage} className="h-2 mt-2" />
            ...
            <CardContent className="p-4">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{metrics.performance.cacheHitRatio}%</p>
                  <Progress value={metrics.performance.cacheHitRatio} className="h-2 mt-2" />
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <Badge variant={metrics.performance.slowQueries > 10 ? "destructive" : "secondary"}>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-mono">{metrics.performance.avgQueryTime}ms</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">Ratio de aciertos de caché</span>
                <span className="text-sm font-mono">{metrics.performance.cacheHitRatio}%</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">Consultas por segundo</span>
                <span className="text-sm font-mono">{metrics.performance.queriesPerSecond}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-mono">{metrics.health.uptime}</span>
              </div>

              <div className="flex justify-between items-center">
                <span className="text-sm font-medium">Último backup</span>
                <span className="text-sm font-mono">{metrics.health.lastBackup.toLocaleDateString()}</span>
              </div>

              {metrics.health.replicationLag !== undefined && (
                <div className="flex justify-between items-center">
                  <span className="text-sm font-medium">Lag de replicación</span>
                  <Badge variant={metrics.health.replicationLag > 1000 ? "destructive" : "secondary"}>
              ...
              <div className="flex justify-between items-center">
                ...
                <Badge variant={metrics.errors.recent > 0 ? "destructive" : "secondary"}>{metrics.errors.recent}</Badge>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="text-center p-4 border rounded">
                <div className="text-2xl font-bold">{metrics.connections.total}</div>
            ...
            <CardContent>
              <div className="space-y-2">
                {metrics.health.status !== "healthy" && (
                  <p className="text-sm text-yellow-800">• Estado de la base de datos: {metrics.health.status}</p>
                )}
                {metrics.errors.recent > 0 && (
                  <p className="text-sm text-yellow-800">• Se detectaron {metrics.errors.recent} errores recientes</p>
                )}
                {metrics.performance.slowQueries > 10 && (
                  <p className="text-sm text-yellow-800">
                    • Número elevado de consultas lentas: {metrics.performance.slowQueries}
                  </p>
                )}
                {metrics.storage.usagePercentage > 80 && (
                  <p className="text-sm text-yellow-800">
                    • Uso de almacenamiento alto: {metrics.storage.usagePercentage}%
                  </p>
                )}
              </div>
            </CardContent>
