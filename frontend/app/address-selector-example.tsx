...
import AddressSelector from "../components/address-selector.tsx";
import type { Address, AddressSelectorProps } from "../types/address-selector.ts";

// Definir 'setAddresses'
const setAddresses = (updateFn: (prev: Address[]) => Address[]) => {
...
const AddressSelectorExample = ({ addresses, selectedAddressId, onSelectAddress }: AddressSelectorProps) => {
	const [selectedAddressIdState, setSelectedAddressId] = useState<string>(addresses.find((addr: Address) => addr.isDefault)?.id || "");
	const [isLoading, setIsLoading] = useState(false);
	const [orderStep, setOrderStep] = useState<"address" | "confirmation">("address");

	const selectedAddress = addresses.find((addr: Address) => addr.id === selectedAddressIdState);
	...
	const handleAddAddress = async (newAddress: Omit<Address, "id">) => {
		...
		if (address.isDefault) {
			setAddresses((prev) => prev.map((addr: Address) => ({ ...addr, isDefault: false })));
		}
	...
	const handleDeleteAddress = async (addressId: string) => {
		...
		setAddresses((prev) => prev.filter((addr: Address) => addr.id !== addressId));
