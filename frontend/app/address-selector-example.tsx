...
} from '../types/address-selector.ts';
...
const sampleAddresses: Address[] = [
  ...
  },
  {
    ...
    instructions:
      'Horario de oficina: 9:00 - 18:00. Preguntar por María en recepción.',
  },
  {
    id: '3',
...
const AddressSelectorExample = ({
  ...
  const [selectedAddressIdState, setSelectedAddressId] = useState<string>(
    addresses.find((addr: Address) => addr.isDefault)?.id || ''
  );
  const [isLoading, setIsLoading] = useState(false);
  const [orderStep, setOrderStep] = useState<'address' | 'confirmation'>(
    'address'
  );

  const selectedAddress = addresses.find(
    (addr: Address) => addr.id === selectedAddressIdState
  );
  ...
  const handleDeleteAddress = async (addressId: string) => {
    ...
    if (selectedAddressId === addressId) {
      ...
      setSelectedAddressId(
        defaultAddress?.id || remainingAddresses[0]?.id || ''
      );
    }
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Ejemplo del componente AddressSelector para gestionar direcciones de
          entrega
        </p>
      </div>
